...
| `:review` · `:review ‹rev›` | an **adversarial defect review** of your working changes (or a branch/range): severity-ranked findings anchored to `file:line`, each with a concrete failure scenario and suggested fix, grounded in the repo map (the changed code *and its callers*). Findings auto-save under `reviews/`. Where `:diff` explains a change, `:review` tries to break it |
| `:tests` | writes **reproduction test files** for the last `:review`'s findings and stages them as **real files** under the notes root (`reproduction/‹rev›/…`, mirroring their repo paths — one `cp -R` drops them all in). The full output is also saved beside the review (`reviews/‹rev›-tests.md`). A test that FAILS confirms its finding was real; one that passes means the finding was likely noise. d-code never writes to your source tree — a review finding is an *opinion*, a reproduction test makes it a *checkable claim* |
| `:verify` · `:verify!` | runs the project's **test suite** — the *evidence* rung. Bare `:verify` is the consent gate: it shows the detected command (`go test ./...`, `cargo test`, `npm test`, `pytest`, `make test` — or name one: `:verify! ‹command›`) and runs nothing. `:verify!` executes it in the project root, streaming live output into the chat (`Esc` stops it, 10-minute cap). A failing run gets an AI **interpretation** grounded in the output and your working diff; offline, the run still works — only the interpretation needs a provider. This is the one place d-code executes project code, and only ever behind the `!` |
